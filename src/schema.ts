...
type SchemaInput = Schema<any> | JsonTypeName
...
interface Schema<K extends Keywords> {
  ...
  const<Const extends AnyJson>(c: Const): Update<K, Keyword.Const<Const>>['calc']
  ...
  required<Keys extends string>(...k: Keys[]): Update<K, Keyword.Required<Keys>>['calc']
